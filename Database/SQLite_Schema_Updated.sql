-- HD Scheduler SQLite Database Schema - UPDATED
-- Split Patient table into Patients (demographics) and HDSchedule (session data)

-- Drop existing tables if they exist
DROP TABLE IF EXISTS PostDialysisMedications;
DROP TABLE IF EXISTS IntraDialyticRecords;
DROP TABLE IF EXISTS HDLogs;
DROP TABLE IF EXISTS HDSchedule;
...
-- Create Patients Table (Demographics Only)
...
-- Create HDSchedule Table (Hemodialysis Session Data)
CREATE TABLE HDSchedule (
    ...
    -- Prescription
    PrescribedDuration REAL,        -- in hours
    UFGoal REAL,                    -- Ultrafiltration goal in liters
    DialysatePrescription TEXT,     -- Normal / K+ Free / Ca++ / Dextrose
    PrescribedBFR INTEGER,          -- Blood Flow Rate in mL/min
    
    -- Anticoagulation
    AnticoagulationType TEXT,       -- Heparin / Without Heparin
    HeparinDose REAL,
    SyringeType TEXT,               -- 10/20 ml
    BolusDose REAL,                 -- in ml
    HeparinInfusionRate REAL,       -- in ml/hr
    
    -- Access Type
    AccessType TEXT,                -- AVF/AVG/CVC
...
-- Create HDLogs Table (Historical dialysis session logs)
CREATE TABLE HDLogs (
    LogID INTEGER PRIMARY KEY AUTOINCREMENT,
    PatientID INTEGER NOT NULL,
    SessionDate TEXT NOT NULL,
...
CREATE INDEX idx_patients_phone ON Patients(PhoneNumber1);
...
SELECT 'Updated SQLite database schema created successfully!' AS Message;
